export const HeroBannerSection = (): JSX.Element => {
  ...
  return (
    <section className="w-full flex justify-center relative bg-black">
      {/* Clouds background image */}
      <img
        className="absolute top-0 left-0 w-full h-full object-cover pointer-events-none z-0 opacity-30"
        alt="Clouds background"
        src="/clouds.webp"
      />

      <div className="flex flex-col items-center justify-center gap-2.5 w-full max-w-[1439px] relative z-10">
        {/* Logo area with wings */}
        <div className="relative w-[580px] h-[277px] flex-shrink-0">
          {/* Right wing */}
          <div className="absolute top-[25px] left-[427px] w-[153px] h-[165px] bg-[url(/wing.png)] bg-[100%_100%]" />
          {/* Left wing */}
          <div className="absolute top-[25px] left-0 w-[153px] h-[165px] bg-[url(/wing-1.png)] bg-[100%_100%]" />
          {/* Central logo */}
          <div className="absolute top-0 left-[97px] w-[406px] h-[243px] bg-[url(/toilogo.png)] bg-cover bg-[50%_50%]" />
        </div>

        {/* Stats section */}
        <div className="flex flex-col w-full items-center relative">
          {/* Decorative vector lines */}
          <div className="relative w-[620px] h-2">
            <img
              className="top-full absolute w-full h-0 left-0"
              alt="Vector"
              src="/vector-249.svg"
            />
            <img
              className="top-0 absolute w-full h-0 left-0"
              alt="Vector"
              src="/vector-249.svg"
            />
          </div>
          ...
          {/* Stats row */}
          <div className="flex items-center justify-center relative self-stretch w-full mt-[-46px]">
            {/* КОМАНДЫ stat */}
        ...
        <div className="inline-flex h-[117px] items-center justify-center gap-2.5 px-[70px] py-[52px] relative">
          <div className="relative flex items-center justify-center w-fit mt-[-79.50px] mb-[-77.50px] bg-[linear-gradient(134deg,rgba(255,6,0,1)_25%,rgba(66,2,0,1)_76%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Kudry-WeirdHeadline',Helvetica] text-transparent text-[70px] tracking-[2.10px] leading-[170px] whitespace-nowrap font-normal text-center">
            УЧАСТВОВАТЬ
          </div>
        </div>
